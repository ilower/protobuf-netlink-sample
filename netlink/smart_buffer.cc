...
void SmartBuffer::consume(int size) {
    if(size > _usedSize || size < 0)
    {
        throw Exception(Exception::OUT_OF_RANGE, "SmartBuffer::consume index out of range");
    }
    if(size == 0)
    {
        return;
    }
    reserve(_usedSize-size);
}

void SmartBuffer::reserve(int size) {
    if(size > _usedSize || size < 0) {
        throw Exception(Exception::OUT_OF_RANGE, "SmartBuffer::reserv index out of range");
    }
