...
//zhangqi, 2015-08-20, 非阻塞模式的connect，避免断网时连游戏服的connect默认70多秒超时卡住UI影响体验
int Socket::connect_noblock(struct addrinfo *res) {
    //设置socket非阻塞模式
    int flags = fcntl(_socketHandler, F_GETFL, 0);
    fcntl(_socketHandler, F_SETFL, flags | O_NONBLOCK);
    
    int conRet = connect(_socketHandler, res->ai_addr, res->ai_addrlen);
    if (conRet == 0) {
        fcntl(_socketHandler, F_SETFL, flags); //connect 成功，恢复socket的默认模式
        return 0;
    } else if (conRet < 0 && errno != EINPROGRESS){ //EINPROGRESS,  表示连接建立已经启动但是尚未完成。这是期望的结果，不是真正的错误
        return -1; //连接错误
    }
    
    fd_set wset;
    FD_ZERO(&wset);
    FD_SET(_socketHandler,&wset);
    ...
    int selRet = select(_socketHandler+1, NULL, &wset, NULL,&tval);
    if (selRet == 0) {
        return -1; //连接超时
    }
    
    if (FD_ISSET(_socketHandler,&wset)) {
        int error;
        socklen_t len = sizeof(error);

        //如果连接发生错误，socket 也是可读和可写的，所以需要再次调用 getsockopt 检测是否出错
        int ret = getsockopt(_socketHandler,SOL_SOCKET,SO_ERROR,&error,&len);
        if( ret < 0 || error) {
            return -1; //连接错误
        }
    } else {
    ...
    //恢复socket的默认模式
    fcntl(_socketHandler, F_SETFL, flags);
    return 0;
}
...
//                        status = connect(_socketHandler, res->ai_addr, res->ai_addrlen);
                        status = connect_noblock(res); //zhangqi, 2015-08-20
                        if(status != -1)
                            connected = true;
                        else
                            close(_socketHandler);
...
Socket* Socket::accept() {
    ...
    int new_handler = ::accept(_socketHandler, (struct sockaddr *)&incoming_addr, (socklen_t*)&addrSize);
...
//zhangqi, 2015-07-17
void Socket::sendWithFlag(const void* buffer, size_t size, int flag) {
    ...
    while (sentData < size) {
        int status = ::send(_socketHandler, (const char*)buffer + sentData, size - sentData, flag);
