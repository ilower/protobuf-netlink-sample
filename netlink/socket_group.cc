...
bool SocketGroup::listen(int milisec, void* reference) {
    ...
    if(milisec >= 0) {
        ...
        status = select(maxHandle + 1, &setSockets, NULL, NULL, &timeout);
    } else {
        status = select(maxHandle + 1, &setSockets, NULL, NULL, NULL);
    }
